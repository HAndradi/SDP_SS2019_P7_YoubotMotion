...
class VelocityCommander:
    def __init__(self):
		...
		self.velocity_pub = rospy.Publisher('~velocity_command', TwistStamped, queue_size=1)
    ...
    def check_goal_request(self):
        if self.target_pose is not None:
			self.event_out.publish('Command received -- Executing')
            self.move_to_pose()
		else:
			self.event_out.publish('No command received -- On Standby')
    ...
    def move_to_pose(self):
        ...
        while distance_trans > self.goal_tolerance:

			if self.event_in is 'e_stop':
				self.event_out.publish('Action cancelled -- Aborting')
				break
